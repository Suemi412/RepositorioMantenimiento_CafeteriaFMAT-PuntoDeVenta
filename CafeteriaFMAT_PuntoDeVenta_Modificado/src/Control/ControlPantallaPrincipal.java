...
public class ControlPantallaPrincipal implements ActionListener {
    ...
    public void actionPerformed(ActionEvent evento) {
        if (vistaPrincipal.getMenuAgregarCliente() == evento.getSource() ||
            ...
            v.getBotonBuscar().setVisible(false);
            v.getBotonActualizarSaldo().setVisible(false);
            v.getBotonEliminar().setVisible(false);
            v.getBotonReporte().setVisible(false);
            Cliente client = new Cliente();
            ControlCliente c = new ControlCliente(client, v, this.vistaPrincipal);
            v.setVisible(true);
        }

        if (vistaPrincipal.getBotonBusquedaCliente() == evento.getSource() ||
                vistaPrincipal.getMenuBuscarCliente() == evento.getSource()) {
            this.vistaPrincipal.setEnabled(false);
            vistaClientes v = new vistaClientes();
            v.getBotonAgregar().setVisible(false);
            v.getTextoNombre().setEnabled(false);
            v.getTextoSaldo().setEnabled(false);
            Cliente client = new Cliente();
            ControlCliente c = new ControlCliente(client, v, this.vistaPrincipal);
            v.setVisible(true);
        }

        if (vistaPrincipal.getMenuReporteCliente() == evento.getSource()) {
            try {
                ...
                int cuenta = Integer.valueOf(JOptionPane.showInputDialog("Ingrese la cuenta del cliente"));
        ...
        if (vistaPrincipal.getBotonNuevaVenta() == evento.getSource()
                || vistaPrincipal.getMenuNuevaVenta() == evento.getSource()) {
            this.vistaPrincipal.setEnabled(false);
        ...
        if (vistaPrincipal.getBotonAbrirCorte() == evento.getSource()
                || vistaPrincipal.getMenuAbrirCorte() == evento.getSource()) {
            this.vistaPrincipal.setEnabled(false);
            vistaAbrirCorte vAC = new vistaAbrirCorte();
            ControlAbrirCorte v = new ControlAbrirCorte(mD, this.vistaPrincipal, vAC);
            vAC.setVisible(true);
        ...
        if (vistaPrincipal.getBotonCerrarCorte() == evento.getSource()
                || vistaPrincipal.getMenuCerrarCorte() == evento.getSource()) {
            mD.setDia(null);
            mD.setMes(null);
            mD.setAnio(null);
